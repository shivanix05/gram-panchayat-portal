...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_status') {
    ...
    if (in_array($status, $allowed)) {
        ...
        setFlash('success', "Complaint #$cid status updated to '$status'.");
    }
    redirect('view_complaints.php');
}

// ── Delete Complaint ─────────────────────────────────────────
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    $cid = (int)$_GET['delete'];
    mysqli_query($conn, "DELETE FROM complaints WHERE id=$cid");
    setFlash('success', 'Complaint deleted successfully.');
    redirect('view_complaints.php');
}
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            ?>
                            <span class="badge <?= $cls ?>"><?= $icon ?> <?= $c['status'] ?></span>
                        </td>
                        <td>
